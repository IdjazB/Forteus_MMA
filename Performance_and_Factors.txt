...
names = ["Idjaz Burthen", "Thomas Chladek"]
usernames = ["idjazb", "tchladek"]

config_file_path = Path(__file__).parent / "config.yaml"
with config_file_path.open('rb') as file:
    config = yaml.load(file, Loader=SafeLoader)

hashed_passwords_file_path = Path(__file__).parent / "hashed_passwords.yaml"
with hashed_passwords_file_path.open('rb') as file:
    hashed_passwords = yaml.load(file, Loader=SafeLoader)
...
authenticator = stauth.Authenticate(credentials,"streamlitdashboard", "forteus")
...
if authentication_status:
    ...
    tab1,tab2 = st.tabs(["Performance", "Risk Factors (Eltican)"])
    ...
    with tab1:
        ...
        def main6():
            ...
            plots_and_titles = [
                (plot1, "Crypto Total Marketcap"),
                (plot2, "Bitcoin Dominance")
            ]
        ...
        def create_plot50():
            ...
            btc_premium_table = pd.DataFrame({
                'Period': ['MTD', 'QTD', 'YTD'],
                'premium': [last_month_premium_btc_str, last_quarter_premium_btc_str, last_year_premium_btc_str],
        ...
        def create_plot51():
            ...
            eth_premium_table = pd.DataFrame({
                'Period': ['MTD', 'QTD', 'YTD'],
                'premium': [last_month_premium_eth_str, last_quarter_premium_eth_str, last_year_premium_eth_str],
                'Date': [last_month_date.strftime("%d %b %Y"), last_quarter_date.strftime("%d %b %Y"), last_year_date.strftime("%d %b %Y")],
                'Δ': [f'{last_month_change:.2f}%', f'{last_quarter_change:.2f}%', f'{last_year_change:.2f}%']
            })
        ...
        def main():
            ...
            # Define a list of dictionaries containing plot objects and their titles
            plots_and_titles = [
                (plot50, "Daily GBTC Premium"),
                (plot51, "Daily ETHE Premium")
            ]
    ...
    with tab2:
        ...
        def create_plot45():
            ...
            data_risk = requests.get('https://reporting-api.eltican.com/risk_factors_perf', auth=basic).json()
        ...
        def create_plot46():
            basic = HTTPBasicAuth('numeus', '3upqwx0@Y67he7XD')
            data_exposures_counterparty = requests.get('https://reporting-api.eltican.com/exposures', auth=basic).json()
        ...
        def create_plot47():
            basic = HTTPBasicAuth('numeus', '3upqwx0@Y67he7XD')
            data_exposures_coin = requests.get('https://reporting-api.eltican.com/exposures', auth=basic).json()
