...
bucket_name = "forteus-media-research"
project_path = "public"
project_data_dir = "mma_reporting"
session = boto3.Session(profile_name='forteus-res')
...
def show_events():
    ...
    try:
        df_events = wr.s3.read_excel(
            f"s3://{bucket_name}/{project_path}/{project_data_dir}/output_events_{end_date}.xlsx",
    ...
    # If the Excel file for the specified end date is not available, read from the previous day
        df_events = wr.s3.read_excel(
            f"s3://{bucket_name}/{project_path}/{project_data_dir}/output_events_{previous_date}.xlsx",
                boto3_session=session
        )
...
def show_tasks():
    ...
    try:
        df_tasks = wr.s3.read_excel(
            f"s3://{bucket_name}/{project_path}/{project_data_dir}/output_tasks_{end_date}.xlsx",
            boto3_session=session
        )
    ...
    # If the Excel file for the specified end date is not available, read from the previous day
        df_tasks = wr.s3.read_excel(
            f"s3://{bucket_name}/{project_path}/{project_data_dir}/output_tasks_{previous_date}.xlsx",
                boto3_session=session
        )
...
def show_selection_status():
    ...
    try:
        df_selection = wr.s3.read_excel(
            f"s3://{bucket_name}/{project_path}/{project_data_dir}/output_selection_{end_date}.xlsx",
            boto3_session=session
        )
    ...
    # If the Excel file for the specified end date is not available, read from the previous day
        df_selection = wr.s3.read_excel(
            f"s3://{bucket_name}/{project_path}/{project_data_dir}/output_selection_{previous_date}.xlsx",
                boto3_session=session
        )
