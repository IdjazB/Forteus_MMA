...
with tab2:
    ...
    # Global variable to store the pivot table
    grouped_df_counterparty_pivot = None
    initial_timestamp = None 
    data_frames = []

    # Function to fetch data from the API and update the DataFrame
    def update_dataframe():
        global grouped_df_counterparty_pivot, data_frames, initial_timestamp
        ...
        grouped_df_counterparty = df_exposures_counterparty.groupby('counterparty')['exposure'].sum().reset_index()
            ...
            # If data_frames is empty, store the initial timestamp
        if not data_frames:
            initial_timestamp = datetime.datetime.now(pytz.utc)

        timestamp = datetime.datetime.now(pytz.utc)

        # Insert the timestamp as the first column for subsequent rows
        if not data_frames:
            grouped_df_counterparty.insert(0, 'Timestamp', initial_timestamp)
        else:
            grouped_df_counterparty.insert(0, 'Timestamp', timestamp)

        # Concatenate the new data to the list
        data_frames = [grouped_df_counterparty]

        # Concatenate all the DataFrames in the list
        grouped_df_counterparty_pivot = pd.concat(data_frames)

        # Pivot the DataFrame to get exposure values for each counterparty as columns
        grouped_df_counterparty_pivot = grouped_df_counterparty_pivot.pivot(index='Timestamp', columns='counterparty', values='exposure')

        # Clear the list for the next 5-minute interval
        data_frames = []

    # Schedule the update_dataframe function to be called every 5 minutes
    schedule.every(5).minutes.do(update_dataframe)
    ...
    # Run the initial DataFrame update
    update_dataframe()
    ...
    if plot45 is not None and plot46 is not None and plot47 is not None:
        ...
        # Display the DataFrame (if it's not None)
        if grouped_df_counterparty_pivot is not None:
            col2.plotly_chart(plot46, use_container_width=True)
            col2.dataframe(grouped_df_counterparty_pivot, use_container_width=True)
