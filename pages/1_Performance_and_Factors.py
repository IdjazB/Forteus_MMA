...
with tab1:
    ...
    def create_plot50():
        url50 = "https://www.theblock.co/api/charts/chart/crypto-markets/structured-products/premium-of-gbtc"
        r50 = session1.get(url50)
        r_json50 = r50.json()

        output_dataframe50 = pd.DataFrame()
        series_dict50 = r_json50['chart']['jsonFile']['Series']
        for vol in series_dict50:
            df_ = pd.DataFrame(series_dict50[vol]['Data']).rename(columns={'Timestamp': 'date', 'Result': 'Premium'})
            df_['date'] = df_['date'].apply(lambda x: datetime.datetime.fromtimestamp(x, tz=datetime.timezone.utc).strftime("%d %b %Y"))
            output_dataframe50 = pd.concat([df_, output_dataframe50])

        output_dataframe50['date'] = pd.to_datetime(output_dataframe50['date'])

        # Calculate the "All-Time High" and "Current Market Cap" values
        current_premium_btc = output_dataframe50['Premium'].iloc[-1]

        fig50 = px.area(output_dataframe50, x='date', y='Premium', title='Daily GBTC Premium')
        fig50.update_layout(
            xaxis_title="Date",
            yaxis_title="Premium (%)",
            hovermode="x"
        ...
        # Update the line color to the custom color
        fig50.update_traces(line_color="#7030A0")

        fig50.update_traces(
            hovertemplate='<b>%{x|%d-%b-%y}</b><br>Premium: %{y:.2f}%'
        )

        # Format y-axis tick labels as percentages
        fig50.update_yaxes(ticksuffix="%")

        # Calculate the quarterly, monthly, and yearly maximum dominance values

        last_year_premium_btc = output_dataframe50.loc[output_dataframe50['date'].dt.year == last_year_date.year, 'Premium'].values[-1]
        last_month_premium_btc = output_dataframe50.loc[output_dataframe50['date'].dt.year == last_month_date.year]
        last_month_premium_btc = last_month_premium_btc.loc[last_month_premium_btc['date'].dt.month == last_month_date.month]
        last_month_premium_btc = last_month_premium_btc['Premium'].values[-1]

        last_quarter_premium_btc = output_dataframe50.loc[(output_dataframe50['date'].dt.date >= last_quarter_start_date) & (output_dataframe50['date'].dt.date <= last_quarter_date)]
        last_quarter_premium_btc = last_quarter_premium_btc['Premium'].values[-1]
        last_week_premium_btc = output_dataframe50.loc[(output_dataframe50['date'].dt.date >= last_week_start_date) & (output_dataframe50['date'].dt.date <= last_week_date)]
        last_week_premium_btc = last_week_premium_btc['Premium'].values[-1]

        # Format dominance values as percentages
        current_premium_btc_str = f'{current_premium_btc:.2f}%'
        last_week_premium_btc_str = f'{last_week_premium_btc:.2f}%'
        last_month_premium_btc_str = f'{last_month_premium_btc:.2f}%'
        last_year_premium_btc_str = f'{last_year_premium_btc:.2f}%'
        last_quarter_premium_btc_str = f'{last_quarter_premium_btc:.2f}%'

        # Calculate the change values as percentages
        last_week_change = current_premium_btc - last_week_premium_btc
        last_month_change = current_premium_btc - last_month_premium_btc
        last_year_change = current_premium_btc - last_year_premium_btc
        last_quarter_change = current_premium_btc - last_quarter_premium_btc

        # Create the dominance table
        btc_premium_table = pd.DataFrame({
            'Period': ['WTD', 'MTD', 'QTD', 'YTD'],
            'premium': [last_week_premium_btc_str, last_month_premium_btc_str, last_quarter_premium_btc_str, last_year_premium_btc_str],
            'Date': [last_week_date.strftime("%d %b %Y"), last_month_date.strftime("%d %b %Y"), last_quarter_date.strftime("%d %b %Y"), last_year_date.strftime("%d %b %Y")],
            'Δ': [f'{last_week_change:.2f}%', f'{last_month_change:.2f}%', f'{last_quarter_change:.2f}%', f'{last_year_change:.2f}%']
        })

        fig50.add_annotation(
            xref="paper",
            yref="paper",
            x=0.98,
            y=0.95,
            xanchor="right",
            yanchor="top",
            text=f"Current GBTC Premium: {current_premium_btc:.2f}%",
            showarrow=False,
            align="right",
            font=dict(
                family="Arial",
        ...
        return fig50, btc_premium_table
    
    def create_plot51():
        url51 = "https://www.theblock.co/api/charts/chart/crypto-markets/structured-products/premium-of-ethe"
        r51 = session1.get(url51)
        r_json51 = r51.json()

        output_dataframe51 = pd.DataFrame()
        series_dict51 = r_json51['chart']['jsonFile']['Series']
        for vol in series_dict51:
            df_ = pd.DataFrame(series_dict51[vol]['Data']).rename(columns={'Timestamp': 'date', 'Result': 'Premium'})
            df_['date'] = df_['date'].apply(lambda x: datetime.datetime.fromtimestamp(x, tz=datetime.timezone.utc).strftime("%d %b %Y"))
            output_dataframe51 = pd.concat([df_, output_dataframe51])

        output_dataframe51['date'] = pd.to_datetime(output_dataframe51['date'])

        # Calculate the "All-Time High" and "Current Market Cap" values
        current_premium_eth = output_dataframe51['Premium'].iloc[-1]

        fig51 = px.area(output_dataframe51, x='date', y='Premium', title='Daily ETHE Premium')
        fig51.update_layout(
            xaxis_title="Date",
            yaxis_title="Premium (%)",
            hovermode="x"
        ...
        # Update the line color to the custom color
        fig51.update_traces(line_color="#7030A0")
        ...
        # Format y-axis tick labels as percentages
        fig51.update_yaxes(ticksuffix="%")

        # Calculate the quarterly, monthly, and yearly maximum dominance values

        last_year_premium_eth = output_dataframe51.loc[output_dataframe51['date'].dt.year == last_year_date.year, 'Premium'].values[-1]
        last_month_premium_eth = output_dataframe51.loc[output_dataframe51['date'].dt.year == last_month_date.year]
        last_month_premium_eth = last_month_premium_eth.loc[last_month_premium_eth['date'].dt.month == last_month_date.month]
        last_month_premium_eth = last_month_premium_eth['Premium'].values[-1]

        last_quarter_premium_eth = output_dataframe51.loc[(output_dataframe51['date'].dt.date >= last_quarter_start_date) & (output_dataframe51['date'].dt.date <= last_quarter_date)]
        last_quarter_premium_eth = last_quarter_premium_eth['Premium'].values[-1]
        last_week_premium_eth = output_dataframe51.loc[(output_dataframe51['date'].dt.date >= last_week_start_date) & (output_dataframe51['date'].dt.date <= last_week_date)]
        last_week_premium_eth = last_week_premium_eth['Premium'].values[-1]

        # Format dominance values as percentages
        current_premium_eth_str = f'{current_premium_eth:.2f}%'
        last_week_premium_eth_str = f'{last_week_premium_eth:.2f}%'
        last_month_premium_eth_str = f'{last_month_premium_eth:.2f}%'
        last_year_premium_eth_str = f'{last_year_premium_eth:.2f}%'
        last_quarter_premium_eth_str = f'{last_quarter_premium_eth:.2f}%'

        # Calculate the change values as percentages
        last_week_change = current_premium_eth - last_week_premium_eth
        last_month_change = current_premium_eth - last_month_premium_eth
        last_year_change = current_premium_eth - last_year_premium_eth
        last_quarter_change = current_premium_eth - last_quarter_premium_eth

        # Create the dominance table
        eth_premium_table = pd.DataFrame({
            'Period': ['WTD', 'MTD', 'QTD', 'YTD'],
            'premium': [last_week_premium_eth_str, last_month_premium_eth_str, last_quarter_premium_eth_str, last_year_premium_eth_str],
            'Date': [last_week_date.strftime("%d %b %Y"), last_month_date.strftime("%d %b %Y"), last_quarter_date.strftime("%d %b %Y"), last_year_date.strftime("%d %b %Y")],
            'Δ': [f'{last_week_change:.2f}%', f'{last_month_change:.2f}%', f'{last_quarter_change:.2f}%', f'{last_year_change:.2f}%']
        })

        fig51.add_annotation(
            xref="paper",
            yref="paper",
            x=0.98,
            y=0.95,
            xanchor="right",
            yanchor="top",
            text=f"Current ETHE Premium: {current_premium_eth:.2f}%",
            showarrow=False,
            align="right",
            font=dict(
                family="Arial",
        ...
        return fig51, eth_premium_table
    
    plot50 = None
    btc_premium_table = None
    plot51 = None
    eth_premium_table = None

    # Define a function to update the plot and table variables
    def update_plots_and_tables():
        global plot50, btc_premium_table
        global plot51, eth_premium_table

        plot50, btc_premium_table = create_plot50()
        plot51, eth_premium_table = create_plot51()
    ...
    if plot50 is not None and plot51 is not None:
        button_ranges_premium_btc = {
        "1W": [datetime.datetime.now() - datetime.timedelta(weeks=1), datetime.datetime.now()],
        "1M": [datetime.datetime.now() - datetime.timedelta(days=30), datetime.datetime.now()],
        "1Q": [datetime.datetime.now() - datetime.timedelta(days=90), datetime.datetime.now()],
        "1Y": [datetime.datetime.now() - datetime.timedelta(days=365), datetime.datetime.now()],
        "3Y": [datetime.datetime.now() - datetime.timedelta(days=1095), datetime.datetime.now()],
        "All": [plot50.data[0].x[0], plot50.data[0].x[-1]]  # Placeholder for 'All' button range
        }

        button_labels_premium_btc = ["1W", "1M", "1Q", "1Y", "3Y", "All"]
        ...
        premium_buttons_btc = [
            {
                ...
                "method": "relayout",
                "args": [
                    {"xaxis": {"range": button_ranges_premium_btc[button_label], "tickformat": "%d-%b-%Y"}}
                ],
                }
            for button_label in button_labels_premium_btc
        ]

        # Create the updatemenus configuration for Dominance Graph
        premium_updatemenus_btc = [{
            "type": "buttons",
            "buttons": premium_buttons_btc,
            "x": 0.14,
            "y": 1.1,
            "xanchor": "center",
            "yanchor": "bottom",
        ...
        # Update the layout for Dominance Graph
        plot50.update_layout(
            title=dict(
                text="Daily GBTC Premium",
                x=0.2,
                y=1,
                xanchor="center",
                yanchor="top"
            ),
            updatemenus=premium_updatemenus_btc
        )

        button_ranges_premium_eth = {
        "1W": [datetime.datetime.now() - datetime.timedelta(weeks=1), datetime.datetime.now()],
        "1M": [datetime.datetime.now() - datetime.timedelta(days=30), datetime.datetime.now()],
        "1Q": [datetime.datetime.now() - datetime.timedelta(days=90), datetime.datetime.now()],
        "1Y": [datetime.datetime.now() - datetime.timedelta(days=365), datetime.datetime.now()],
        "3Y": [datetime.datetime.now() - datetime.timedelta(days=1095), datetime.datetime.now()],
        "All": [plot51.data[0].x[0], plot51.data[0].x[-1]]  # Placeholder for 'All' button range
        }

        button_labels_premium_eth = ["1W", "1M", "1Q", "1Y", "3Y", "All"]

        premium_eth_buttons_eth = [
            {
                "label": button_label,
                "method": "relayout",
                "args": [
                    {"xaxis": {"range": button_ranges_premium_eth[button_label], "tickformat": "%d-%b-%Y"}}
                ],
                }
            for button_label in button_labels_premium_eth
        ]

        # Create the updatemenus configuration for Dominance Graph
        premium_eth_updatemenus = [{
            "type": "buttons",
            "buttons": premium_eth_buttons_eth,
            "x": 0.14,
            "y": 1.1,
            "xanchor": "center",
            "yanchor": "bottom",
        ...
        # Update the layout for Dominance Graph
        plot51.update_layout(
            title=dict(
                text="Daily ETHE Premium",
                x=0.2,
                y=1,
                xanchor="center",
                yanchor="top"
            ),
            updatemenus=premium_eth_updatemenus
        )

        col1, col2 = st.columns(2)

        col1.plotly_chart(plot50, use_container_width=True)
        col1.dataframe(btc_premium_table, hide_index = True)
        col2.plotly_chart(plot51, use_container_width=True)
        col2.dataframe(eth_premium_table, hide_index = True)
