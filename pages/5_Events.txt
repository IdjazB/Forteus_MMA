...
with tab1:
    ...
    @st.cache_data(ttl=3600)
    def load_ccxt_binance_funding_data():
        ...
        btc_funding = exchange.fetch_funding_rate_history(btc_symbol, since=1672588800000, limit=1000)
        time.sleep(5)  # Delay for 5 seconds
        eth_funding = exchange.fetch_funding_rate_history(eth_symbol, since=1672588800000, limit=1000)
        
        def to_datetime(datetime_str):
            return datetime.datetime.strptime(datetime_str, '%Y-%m-%dT%H:%M:%S.%fZ')

        btc_rates = [rate["fundingRate"] for rate in btc_funding]
        eth_rates = [rate["fundingRate"] for rate in eth_funding]
        
        datetimes = [to_datetime(rate["datetime"]) for rate in btc_funding]
        
        data = {'Datetime': datetimes, 'BTCUSDT_FundingRate': btc_rates, 'ETHUSDT_FundingRate': eth_rates}
        df = pd.DataFrame(data)
        
        return df

    def create_combined_plot3(selected_asset):
        ccxt_funding_rate_data = load_ccxt_binance_funding_data()
        ccxt_funding_rate_data= ccxt_funding_rate_data[ccxt_funding_rate_data['Datetime'] > '2023-05-24'] 

        fig3 = go.Figure()

        selected_rates = ccxt_funding_rate_data[['Datetime', f'{selected_asset}_FundingRate']]
        positive_rates = selected_rates[selected_rates[f'{selected_asset}_FundingRate'] > 0]
        negative_rates = selected_rates[selected_rates[f'{selected_asset}_FundingRate'] <= 0]

        # Add positive rates as green bars with a name
        fig3.add_trace(go.Bar(
            x=positive_rates['Datetime'],
            y=positive_rates[f'{selected_asset}_FundingRate'],
            marker_color='#31c3a6',  # Green color for positive rates
            name=f'{selected_asset} Positive Funding Rates'
        ))

        # Add negative rates as red bars with a name
        fig3.add_trace(go.Bar(
            x=negative_rates['Datetime'],
            y=negative_rates[f'{selected_asset}_FundingRate'],
            marker_color='red',  # Red color for negative rates
            name=f'{selected_asset} Negative Funding Rates'
        ))

        #fig3.update_layout(title=f'{selected_asset} Funding Rates', xaxis_title='Date', yaxis_title='Funding Rate')

        fig3.update_layout(
            title=f'{selected_asset} Funding Rates',
            xaxis_title='Date',
        ...
        fig3.update_layout(yaxis_tickformat=".3%")
        fig3.update_traces(hovertemplate='%{y:.6%}')

        x_tickvals = selected_rates['Datetime'][::60]
        x_ticklabels = x_tickvals.dt.strftime('%d-%b-%Y')
        fig3.update_xaxes(tickvals=x_tickvals, ticktext=x_ticklabels)
    ...
    def main9():
        ...
        selected_range6 = st.sidebar.selectbox("Select Timeframe:", ["All", "1W", "1M"])
        ...
        combined_plot.update_xaxes(range=button_ranges_combined[selected_range6])
        combined_plot1.update_xaxes(range=button_ranges_combined[selected_range6])
        combined_plot2.update_xaxes(range=button_ranges_combined[selected_range6])
        combined_plot3.update_xaxes(range=button_ranges_combined[selected_range6])

        # Display the plots
        st.plotly_chart(combined_plot)
        st.plotly_chart(combined_plot3)
        st.plotly_chart(combined_plot2)
        st.plotly_chart(combined_plot1)
