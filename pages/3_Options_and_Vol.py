...
def create_plot6():
    ...
    for token, group in output_dataframe6.groupby('ATM'):
        ...
        indicator_data = {
            ...
            'YTD': current_implied_btc_volatility_str,
    ...
    for token, group in output_dataframe7.groupby('Exp'):
        if token != exclude_token:
            ...
            indicator_data = {
                ...
                'QTD (Δ)': f'{last_quarter_change:.2f}%',
                'YTD': current_btc_option_skew_delta_str,
                'YTD (Δ)': f'{last_year_change:.2f}%'
            }
...
if plot6 is not None and plot7 is not None and plot5 is not None:
    ...
    col2.plotly_chart(plot7, use_container_width=True)
    col2.dataframe(btc_option_skew_delta_table, hide_index = True)

    col1, col2 = st.columns(2)

    col1.plotly_chart(plot5, use_container_width=True)
    col1.dataframe(annualized_btc_volatility_table, hide_index = True)
